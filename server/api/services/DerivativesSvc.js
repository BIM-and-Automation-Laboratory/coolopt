...
class DerivativeSvc extends BaseSvc {
  ...
  async postJob(getToken, payload) {
    ...
    return this._derivativesAPI.translate(
      payload,
      {
  ...
  async getFormats(getToken, opts = {}) {
    ...
    return this._derivativesAPI.getFormats(opts, { autoRefresh: false }, token);
  }
  ...
  async deleteManifest(getToken, urn) {
    ...
    var url =
      `${DerivativeSvc.SERVICE_BASE_URL}/designdata/` + `${urn}/manifest`;
  ...
  getDefaultGuid(getToken, urn, role) {
    return new Promise(async (resolve, reject) => {
      try {
        const manifestRes = await this.getManifest(getToken, urn);

        const derivatives1 = this.findDerivatives(manifestRes.body, {
          type: "geometry",
          role: role || "3d",
        });
        ...
        const derivatives2 = this.findDerivatives(manifestRes.body, {
          type: "geometry",
          role: role || "2d",
        });
  ...
  async getThumbnail(
    ...
    const guid =
      options.guid || (await this.getDefaultGuid(token, urn, options.role));
  ...
  findDerivatives(parent, query) {
    ...
    if (derivatives) {
      ...
      const childResults = derivatives.map((derivative) => {
        return this.findDerivatives(derivative, query);
      });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        while (true) {
          ...
          const derivatives = this.findDerivatives(manifest, query);
          ...
          if (derivatives.length) {
            ...
            const status = derivative.status || derivative.parent.status;
          ...
          const parentDerivatives = this.findDerivatives(manifest, {
            outputType,
          });
