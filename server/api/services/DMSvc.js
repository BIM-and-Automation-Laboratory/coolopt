const ServiceManager = require("./SvcManager");
const BaseSvc = require("./BaseSvc");
const Forge = require("forge-apis");
const request = require("request");

class DMSvc extends BaseSvc {
  ...
  async getHubs(getToken, opts = {}) {
    ...
    return this._hubsAPI.getHubs(opts, { autoRefresh: false }, token);
  }
  ...
  async getHub(getToken, hubId) {
    ...
    return this._hubsAPI.getHub(hubId, { autoRefresh: false }, token);
  }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const deleteTasks = versionsRes.body.data.map((version) => {
          return this.deleteVersion(token, projectId, version.id);
        });
  ...
  async deleteVersion(getToken, projectId, versionId) {
    try {
      ...
      const url =
        `${DMSvc.SERVICE_BASE_URL}/projects/` + `${projectId}/versions`;
  ...
  async createStorage(getToken, projectId, folderId, filename) {
    ...
    const payload = this.createStoragePayload(folderId, filename);

    // return this._projectsAPI.postStorage(
    //  projectId, JSON.stringify(payload),
    //  {autoRefresh:false}, token)

    const url = `${DMSvc.SERVICE_BASE_URL}/projects/` + `${projectId}/storage`;
  ...
  async createItem(
    ...
    const url = `${DMSvc.SERVICE_BASE_URL}/projects/` + `${projectId}/items`;
  ...
  async createVersion(
    ...
    const url = `${DMSvc.SERVICE_BASE_URL}/projects/` + `${projectId}/versions`;
  ...
  //
  /// //////////////////////////////////////////////////////////////
  async getItemRelationshipsRefs(getToken, projectId, itemId, opts = {}) {
    const token = typeof getToken === "function" ? await getToken() : getToken;
  ...
  async createItemRelationshipRef(
    ...
    const payload = this.createItemRelationshipRefPayload(refVersionId);
  ...
  //
  /// //////////////////////////////////////////////////////////////
  async getVersionRelationshipsRefs(getToken, projectId, versionId, opts = {}) {
    const token = typeof getToken === "function" ? await getToken() : getToken;
  ...
  async createVersionRelationshipRef(
    ...
    const payload = this.createVersionRelationshipRefPayload(refVersionId);
  ...
  //
  /// //////////////////////////////////////////////////////////////
  async createFolder(getToken, projectId, parentFolderId, folderName) {
    const token = typeof getToken === "function" ? await getToken() : getToken;

    const url = `${DMSvc.SERVICE_BASE_URL}/projects/` + `${projectId}/folders`;

    const payload = this.createFolderPayload(parentFolderId, folderName);
  ...
  upload(getToken, projectId, folderId, file, opts) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const objectId = ossSvc.parseObjectId(storageRes.body.data.id);
        ...
        const uploadRes = await ossSvc.uploadObjectChunked(
          getToken,
          objectId.bucketKey,
          objectId.objectKey,
          file,
  ...
  createItemPayload(folderId, objectId, displayName, isBIM) {
    const fileType = isBIM ? "autodesk.bim360:File" : "autodesk.core:File";
