...
class ForgeSvc extends BaseSvc {
  /// //////////////////////////////////////////////////////
  //
  //
  /// //////////////////////////////////////////////////////
  static get BASE_HOOKS_URL() {
    return "https://developer.api.autodesk.com/webhooks/v1";
  }
  ...
  constructor(config) {
    ...
    this.__refresh3LeggedTokenMemo = memoize(
      (session, scope) => {
        return this.__refresh3LeggedToken(session, scope);
      },
      {
        normalizer: (args) => {
          const memoId = {
  ...
  async getUser(session) {
    ...
    const token = await this.get3LeggedTokenMaster(session);
  ...
  get2LeggedToken() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!token) {
          token = await this.request2LeggedToken(this._config.oauth.scope);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (this.getExpiry(token) < 60) {
          ...
          this.set3LeggedTokenMaster(session, token);
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!session.forge) {
          ...
          const clientToken = await this.refresh3LeggedToken(session, scopeStr);

          this.set3LeggedTokenClient(session, clientToken);
        }

        var token = session.forge.clientToken;

        if (this.getExpiry(token) < 60) {
          token = await this.refresh3LeggedToken(session, scopeStr);

          this.set3LeggedTokenClient(session, token);
        }
  ...
  refresh3LeggedToken(session, requestedScope) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        while (true) {
          token = await this.__refresh3LeggedTokenMemo(session, requestedScope);
  ...
  //
  /// //////////////////////////////////////////////////////
  generateCryptoToken({ stringBase = "base64", byteLength = 48 } = {}) {
  ...
  __refresh3LeggedToken(session, scope) {
    return new Promise((resolve, reject) => {
      const url =
        this._config.oauth.baseUri + this._config.oauth.refreshTokenUri;
      ...
      request(
        ...
        function (err, response, body) {
          try {
            ...
            if (
              response &&
              [200, 201, 202, 204].indexOf(response.statusCode) < 0
            ) {
  ...
  getSystemHooks(token, systemId) {
    const url = `${ForgeSvc.BASE_HOOKS_URL}/systems/${systemId}/hooks`;
