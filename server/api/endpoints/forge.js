...
module.exports = function() {
  ...
  router.get("/token/3legged", async (req, res) => {
    ...
    try {
      const scope = ["viewables:read", "data:write", "data:read"];

      const token = await forgeSvc.get3LeggedTokenClient(req.session, scope);
  ...
  router.get("/token/2legged", async (req, res) => {
    ...
    try {
      const token = await forgeSvc.request2LeggedToken("viewables:read");
