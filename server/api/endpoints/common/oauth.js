...
function getClient(scopes) {
  const { client_id, client_secret } = config.forge.oauth;
  return new AuthClientTwoLegged(
    client_id,
    client_secret,
    scopes || config.forge.oauth.scopes.internal
  );
}
...
async function getPublicToken() {
  return getToken(config.forge.oauth.scopes.public);
}
...
async function getInternalToken() {
  return getToken(config.forge.oauth.scopes.internal);
}
