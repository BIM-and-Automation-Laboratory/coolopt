...
const mongo = require("mongodb"); //FIXME: 
...
module.exports = function() {
  ...
  router.get("/formats", async (req, res) => {
    try {
      ...
      const derivativesSvc = ServiceManager.getService("DerivativesSvc");
  ...
  router.get("/metadata/:urn", async (req, res) => {
    try {
      ...
      const response = await derivativesSvc.getMetadata(token, urn);
  ...
  router.get("/manifest/:urn", async (req, res) => {
    try {
      ...
      const response = await derivativesSvc.getManifest(token, urn);
  ...
  router.get("/hierarchy/:urn/:guid", async (req, res) => {
    try {
      ...
      const response = await derivativesSvc.getHierarchy(token, urn, guid);
  ...
  router.get("/properties/:urn/:guid", async (req, res) => {
    try {
      ...
      const response = await derivativesSvc.getProperties(token, urn, guid, {
        objectId: objectId,
      });
  ...
  router.get("/thumbnails/:urn", async (req, res) => {
    try {
      ...
      const response = await derivativesSvc.getThumbnail(token, urn, options);
  ...
  router.post("/job/:db/:modelId", async (req, res) => {
    try {
      const userSvc = ServiceManager.getService("UserSvc");

      const user = await userSvc.getCurrentUser(req.session);
      ...
      // FIXME: Inject Neo instead of MongoDB
      const db = req.params.db;
      
      const modelsConfig = config.database.models[db];
      ...
      const modelSvc = ServiceManager.getService(db + "-ModelSvc");
      ...
      const response = await derivativesSvc.postJobWithProgress(token, job, {
        query: {
          outputType: format,
        },
        onProgress: (progress) => {
